...
const Profile = (props) =>{
    ...
    useEffect(()=> {
        ...
        axios.get(`http://localhost:8000/api/ordersByUser/${username}`,  {withCredentials: true})
            .then((res) =>{
    ...
    return(
        <div>
             <Header link={"/"} linkText={"Home"} titleText={"Rent My Car"} loggedIn= {loggedIn} setUser = {setUser} user = {user}/>
            {
              loggedIn ? 
                <div>
                        <h1 class="flex justify-center text-xl m-8">Welcome {user.firstName}</h1>
                                        ...
                                        return(
                                            <div class="flex-row justify-center" key = {index}>
                                                <div class=" flex border-2 border-grey-200 rounded p-2 hover:shadow-xl">
                                                    <div class="w-60">
                                                        <h3 class="font-bold mb-2">{vehicle.make} {vehicle.model}</h3>
                                                        <img src={vehicle.image} alt="Vehicle picture" class = "w-30 h-30" />
                                                ...
                                                <DeleteButton id={vehicle._id} vehicleList={userVehicleList} setVehicleList={setUserVehicleList}/>
                                            </div>
                                        )
                                    })
                                }
                            </div>
                            <button class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 mt-4 border border-blue-500 hover:border-transparent rounded" onClick={(e)=>navigate("/new")}>Add Vehicle</button>
                    </div>
                    <div class="border-2 border-sky-200 rounded m-2 p-3">
                                        ...
                                        return(
                                            <div class="flex-row justify-center" key = {index}>
                                                <div class="flex-column w-60 border-2 border-grey-200 rounded p-2 hover:shadow-xl">
                                                        ...
                                                        <img src={order.image} alt="vehicle image" class = "object-cover w-full"/>
                                                        <p>Order ID: {order._id}</p>
                                                        <p>Cost: ${order.totalCost}</p>
                                                    
                                                </div>
                                                <button class="bg-transparent hover:bg-red-500 text-red-700 font-semibold hover:text-white py-2 px-4 mt-4 border border-red-500 hover:border-transparent rounded" onClick={(e) => deleteHandler(order._id)}>Delete</button>
                                            </div>
                                        )
